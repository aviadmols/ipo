...
// If program_id post type is not program, then return
if(get_post_type($program_id) != 'program'){
	echo '<!-- Program id provided, but post type is not program -->';
	return;
}
...
foreach($events as $event){
	...
	if(ICL_LANGUAGE_CODE == 'he'){
		if($event_obj->get_day() == 'ש'){
			$event_day = 'יום שבת';
		} else {
			$event_day = 'יום ' . $event_obj->get_day() . "'";
		}

	} else {
	$event_day = date('D', (int) $event_date);
	}
	...
	<li data-event_id="'.$event.'" data-original-event-date="'.$event_date.'">
		...
		<div style="display: flex;">
			<span class="location">'.$event_city.'</span> <img src="'. ipo_arrow_icon_url() .'" class="arrow" alt="">
		</div>
		</a>
	</li>';
}
...
$playlist_link = '';
if($events){
	$playlist_link = get_permalink($events[0]).'#playlist';
}
		...
		<div class="img_box">
			...
			<div class="order-1 order-lg-1">
					<div class="position-relative">
						...
						<a class="overlay-link" href="'.get_permalink($program_id).'"></a>
						<a class="playlist" href="'.$playlist_link.'"><img src="/wp-content/uploads/2022/08/small_icons.svg" class="w-100 h-100" alt=""></a>
					</div>
				</div>
